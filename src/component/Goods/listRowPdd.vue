            ...
            <li v-for="(item,index) in list" :key="index" @click="gobuy(item)">
                ...
                <div class="goodsright">
                    <div>
                        ...
                        <div class="goodsyhjdiv" v-if="item.couponFee">
                            <!-- <div class="goodsyhj" v-for="(item2,index2) in item.couponFee" :key="index2">{{item2.coupon_discount}}元优惠券</div> -->
                            <div class="goodsyhj">{{item.couponFee}}元优惠券</div>
                        </div>
                    </div>
                    ...
                    <div class="goodsright_bottom">
                        <div class="goodspricediv">
                            <div class="goodspicediv">
                                <div class="goodspricenow">¥{{item.price}}</div>
                                <div class="goodspriceold">¥{{item.vipPrice}}</div>
                            </div>
                        </div>
...
export default {
    ...
    methods:{
        gobuy(data){ 
            this.$router.push({path:'/goodsDetailCPS?',query:{goodsId:data.productId,status:4}});
        },
        onLoad(){
            this.$emit("getlist")
        }
