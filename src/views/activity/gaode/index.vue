<template>
    <div class="main">
        <div class="maincenter">
            ...
            <img class="quanimg" src="https://yuetao-1300766538.cos.ap-beijing.myqcloud.com/yuetao/image/2020-08-24/20/yuelvhuixjDbWuhRFr1598271962.png">
            <img class="btnimg" @click="lingqu" src="https://yuetao-1300766538.cos.ap-beijing.myqcloud.com/yuetao/image/2020-08-16/17/yuelvhuipvO7WzKEEh1597570841.png">
        </div>
        <div class="bottom">
...
<script>
import {lingQuGaoDeLibao ,isGetGaoDeDaLiBao ,sharePage ,getCodeNumber} from "@/services/activity"
...
export default {
    ...
    methods:{
        isLingqu(){
            ...
            if(uid){
                isGetGaoDeDaLiBao({"uid":uid}).then(res=>{
                    if(res.code==400){//已领取
                        this.$router.replace({path:"/gaodelist"});
                    }
                })
            }
        },
        lingqu(){
            let uid=localStorage.getItem("uid");
            if(uid){
                lingQuGaoDeLibao({"uid":uid}).then(res=>{
                    if(res.code==200){
                        Toast("领取成功");
                        this.$router.replace({path:"/gaodelist"});
                    }else{
                ...
                let codeNumber=this.$route.query.codeNumber;
                if(codeNumber){
                    this.codeNumber=codeNumber;
                    this.$router.push({path:"/login",query:{"active":"gaode","codeNumber":codeNumber}});
                }else{
                    getCodeNumber().then(res=>{
                        if(res.code==200){
                            this.codeNumber=res.data.codeNumber;
                            this.$router.push({path:"/login",query:{"active":"gaode","codeNumber":res.data.codeNumber}});
                        }else{
                            Toast(res.msg);
                        }
                    });
