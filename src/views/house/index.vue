<template>
  ...
  <div>
    ...
    <!--推荐商品-->
    <fresh-list :list="commendList"
                :childLoadParams="loadParams"
                @sendInfo="changePage"></fresh-list>
    <!--底部tab-->
    <Nav></Nav>
  </div>
</template>
...
export default {
  ...
  methods: {
    //下滚页面增加页码请求
    changePage (e) {
      this.loadParams.page = e.nowPage;
      this.getrecommendList();
    },
    getNav () {
      let opt = {
      ...
      getrecommendList(opt).then(res => {
        if (res.code == 200) {
          if (res.data.length < 1 || !res.data) {
            ...
            this.commendList = this.commendList.concat(res.data);
          }
          this.loadParams.loading = false;
        }
      }).catch(error => {
        // console.log(error);
        this.loadParams.error = true;
      }).finally(() => {
        this.loadParams.loading = false;
      })
    },
